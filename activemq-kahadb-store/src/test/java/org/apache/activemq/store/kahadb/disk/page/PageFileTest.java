...
public class PageFileTest {
    ...
    @Test
    public void readFreePagesFromFreePageFileAndImportToFreePageMap() throws Exception {
        PageFile pf = new PageFile(new File("target/test-data"), getName());
        pf.delete();
        ...
        assertEquals(400, pf2.getPageCount());
        // With recovery=false, the background recovery will not have time to run and the number of free pages will be 0
        assertEquals(recoveryFile ? 100 : 0, pf2.getFreePageCount());

        for (int i = 0; i < 400; i++) {
            assertEquals(pf2.isFreePage(i), i % 4 == 0 && recoveryFile);
        }
    }
